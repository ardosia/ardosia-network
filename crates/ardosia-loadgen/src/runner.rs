...
pub async fn run_profile(
    ...
    let Some(capture) = capture else {
        let metadata = ProfileMetadata::from_failure(
            ...
            report.environment.raknet_upstream_revision.clone(),
            report.environment.raknet_hardfork_revision.clone(),
            "profiling".into(),
            0,
            validated_perf_version,
            config.frequency_hz,
...
fn write_early_profile_failure(
    ...
    let metadata = ProfileMetadata::from_failure(
        ...
        environment.raknet_upstream_revision,
        environment.raknet_hardfork_revision,
        "profiling".into(),
        0,
        perf_version,
        config.frequency_hz,
